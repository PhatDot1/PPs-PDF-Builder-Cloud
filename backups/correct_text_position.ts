...
async function processRecord(record: any) {
    try {
        ...
        // Achievement Level: Fixed above programme name
        const achievementLevelY = programmeNameY - 150; // Adjust as needed for spacing
        background.print(
            fontRegular,
            margin,
            achievementLevelY,
        ...
        // Participant Name: Positioned dynamically above achievement level
        const wrappedParticipantName = wrapText(participantName, fontRegular, width * 0.8);
        const nameLines = wrappedParticipantName.split('\n');
        const nameLineHeight = 20; // Line height for participant name
        const participantNameY = achievementLevelY - nameLines.length * nameLineHeight - 200; // Move upward by increasing the offset
        background.print(
            fontRegular,
            margin,
            participantNameY,
